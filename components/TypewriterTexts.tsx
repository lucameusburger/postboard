...
export default function TypewriterTexts({
    ...
    // Use a ref to store removeNewTextId so we can use it in handleNewText
    const removeNewTextIdRef = useRef<((id: string) => void) | null>(null);

    // Handle new text from realtime subscription
    const handleNewText = useCallback((newText: Post) => {
        ...
        setTimeout(() => {
            removeNewTextIdRef.current?.(String(newText.id));
        }, animationDuration);
    }, [processOverlayQueue]);

    // Use the realtime hook
    const { texts, newTextIds, removeNewTextId } = useRealtimeTexts({
        initialTexts,
        collectionName,
        onNewText: handleNewText,
    });

    // Store removeNewTextId in ref
    useEffect(() => {
        removeNewTextIdRef.current = removeNewTextId;
    }, [removeNewTextId]);

    // Cleanup overlay interval on unmount
    useEffect(() => {
        return () => {
            if (overlayTypewriterIntervalRef.current) {
                clearInterval(overlayTypewriterIntervalRef.current);
